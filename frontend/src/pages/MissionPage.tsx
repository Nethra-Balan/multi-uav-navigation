...
import { FleetList } from "../components/controlRoom";
...
export default function MissionPage() {
  ...
  return <div className="mission-page">
    <section className="mission-stage">
      <div className="mission-overlay"><span className="eyebrow cyan">MISSION CONTROL / 3D SIMULATION</span><strong><i className="online-dot live" />{lifecycle === "completed" ? "OPTIMIZATION COMPLETE" : lifecycle.toUpperCase()}</strong></div>
    ...
    <div className="mission-underbar">
      {result && <section className="mission-info panel fleet-panel"><div className="panel-head"><div><span className="eyebrow">FLEET STATUS</span><h3>Route association</h3></div></div><FleetList rows={result.allocation.map((row) => ({ uav: row.uav, targetCount: row.target_count, distance: row.path_distance, targets: row.targets }))} lifecycle={lifecycle} onSelect={selectUav} /></section>}
...
function Inspector({ environment, result, selectedUav, selectedTarget, assignedUav, selectedCollision, playback }: { environment: NonNullable<ReturnType<typeof useStore>["environment"]>; result: FinalResult | null; selectedUav: FinalResult["allocation"][number] | undefined; selectedTarget: typeof environment.targets[number] | undefined; assignedUav: number | undefined; selectedCollision: FinalResult["collisions"][number] | undefined; playback: ReturnType<typeof samplePlayback> }) {
  if (selectedUav) return <div className="inspector-list"><p><span>STATUS</span><b className="good">ACTIVE / {result ? "ROUTE LOADED" : "STANDBY"}</b></p><p><span>ASSIGNED TARGETS</span><b>{selectedUav.targets.map((target) => `T${String(target).padStart(2, "0")}`).join(" · ") || "—"}</b></p><p><span>TARGET COUNT</span><b>{selectedUav.target_count}</b></p><p><span>DISTANCE</span><b>{fmt(selectedUav.path_distance)} m</b></p><p><span>POSITION</span><b>{playback.positions[selectedUav.uav]?.map((value) => value.toFixed(1)).join(", ") || "Start position"}</b></p></div>;
  if (selectedTarget) return <div className="inspector-list"><p><span>STATUS</span><b className={playback.visitedTargets.includes(selectedTarget.id) ? "good" : ""}>{playback.visitedTargets.includes(selectedTarget.id) ? "VISITED" : "REMAINING"}</b></p><p><span>POSITION</span><b>[{selectedTarget.position.map((value) => value.toFixed(1)).join(", ")}]</b></p><p><span>ASSIGNED UAV</span><b>{assignedUav ? `UAV-${String(assignedUav).padStart(2, "0")}` : "—"}</b></p></div>;
  if (selectedCollision) return <div className="inspector-list"><p><span>STATUS</span><b className="bad">COLLISION</b></p><p><span>UAV</span><b>UAV-{String(selectedCollision.uav).padStart(2, "0")}</b></p><p><span>SEGMENT</span><b>{selectedCollision.segment_index}</b></p><p><span>OBSTACLE</span><b>OBS-{String(selectedCollision.obstacle).padStart(2, "0")}</b></p><p><span>SEGMENT START</span><b>[{selectedCollision.start.map((value) => value.toFixed(1)).join(", ")}]</b></p></div>;
